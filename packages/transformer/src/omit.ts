import { Bundle, Node, SourceFile, TransformerFactory, TypeElement, factory, isIdentifier, isLiteralTypeNode, isPropertySignature, isStringLiteral, isTypeLiteralNode, isTypeReferenceNode, isUnionTypeNode, visitEachChild, visitNode } from "typescript";
    ...
    const visitor = (node: Node): Node => {
      if (isTypeReferenceNode(node) && isIdentifier(node.typeName) && node.typeName.escapedText === "Omit") {
        if (node.typeArguments) {
          ...
          const keys: string[] = isStringLiteral(omitted) ? [omitted.text] : isUnionTypeNode(omitted) ? omitted.types.map(i => isLiteralTypeNode(i) && isStringLiteral(i.literal) ? i.literal.text : "") : [];
