    ...
    switch (expr.callee.property.name) {
      ...
      case "case":
        if (
          ...
          expr.arguments[1].type === "CallExpression" &&
          expr.arguments[1].callee.type === "Identifier" &&
          expr.arguments[1].callee.name.endsWith("Handler")
        )
          HANDLERS.push({
            type: expr.arguments[1].callee.name,
            arguments: expr.arguments[1].arguments,
            guard: expr.arguments[0].value,
          });
      ...
      for (const decl of node.declaration.declarations)
        if (decl.type === "VariableDeclarator" && decl.id.type === "Identifier") {
          ...
          if (
            ...
            decl.init.callee.property.name === "init"
          )
