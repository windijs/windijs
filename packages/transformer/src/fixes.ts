    ...
    const visitor = (node: Node): Node => {
      if (isTypeAliasDeclaration(node) && isIdentifier(node.name) && node.name.escapedText === "StyleObject")
        return factory.updateTypeAliasDeclaration(
          ...
          node.modifiers,
          node.name,
          [factory.createTypeParameterDeclaration(undefined, "T", undefined, factory.createTypeLiteralNode([]))],
          node.type
        );
      ...
      if (isTypeAliasDeclaration(node) && isIdentifier(node.name) && node.name.escapedText === "StyleProxy")
        return factory.updateTypeAliasDeclaration(
          ...
          [
            ...
            factory.createTypeParameterDeclaration(undefined, "O", undefined, factory.createTypeLiteralNode([])),
          ],
          node.type
        );
      ...
      if (isTypeAliasDeclaration(node) && isIdentifier(node.name) && node.name.escapedText === "BuildFunc")
        return factory.updateTypeAliasDeclaration(
          ...
          node.name,
          [
            factory.createTypeParameterDeclaration(undefined, "V", undefined, factory.createKeywordTypeNode(SyntaxKind.UnknownKeyword)),
