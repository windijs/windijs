  ...
  return sourceFile => {
    const visitor: Visitor = node => {
      if (
        ...
        node.typeName.escapedText === "Pick" &&
        node.typeArguments &&
        isLiteralTypeNode(node.typeArguments[1]) &&
        isStringLiteral(node.typeArguments[1].literal) &&
        node.typeArguments[1].literal.text.startsWith("$windi.")
      )
    ...
    return factory.updateSourceFile(transformed, [
      ...
      factory.createTypeAliasDeclaration(
        ...
        undefined,
        "Inject",
        [factory.createTypeParameterDeclaration(undefined, "A"), factory.createTypeParameterDeclaration(undefined, "B")],
        factory.createConditionalTypeNode(
          factory.createTypeReferenceNode("B"),
          factory.createKeywordTypeNode(SyntaxKind.ObjectKeyword),
          factory.createTypeReferenceNode("B"),
          factory.createTypeReferenceNode("A")
        )
      ),
      ...transformed.statements.filter(i => !isImportDeclaration(i)),
    ]);
