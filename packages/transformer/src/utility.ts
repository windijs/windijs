  ...
  return sourceFile => {
    const visitor = (node: Node): Node => {
      if (isCallExpression(node))
        if (isIdentifier(node.expression)) {
          if (
            (node.expression.escapedText === "configHandler" || node.expression.escapedText === "fontSizeHandler") &&
                  ...
                  factory.createArrowFunction(
                    ...
                    factory.createAsExpression(
                      ...
                      factory.createTypeReferenceNode("Pick", [
                        factory.createTypeReferenceNode("StyleProxy", [
                          factory.createTypeQueryNode(factory.createIdentifier(node.arguments[0].escapedText.toString())),
                        ]),
                        factory.createLiteralTypeNode(
                          factory.createStringLiteral("$windi.config." + node.arguments[0].escapedText.toString() + ".proxy")
                        ),
                      ])
                    )
                  )
                ...
                factory.createPropertyAssignment(
                  ...
                  factory.createArrowFunction(
                    ...
                    factory.createAsExpression(
                      factory.createObjectLiteralExpression([], false),
                      factory.createTypeReferenceNode("Pick", [
                        factory.createTypeReferenceNode("ColorStyleProxy", [
                          factory.createTypeQueryNode(factory.createIdentifier(node.arguments[0].escapedText.toString())),
                        ]),
                        factory.createLiteralTypeNode(
                          factory.createStringLiteral("$windi.color." + node.arguments[0].escapedText.toString() + ".proxy")
                        ),
                      ])
                    )
                  )
          ...
          factory.createObjectLiteralExpression([
            factory.createPropertyAssignment(
              ...
              factory.createAsExpression(
                ...
                factory.createLiteralTypeNode(factory.createStringLiteral("windiColorInject"))
              )
            ),
          ])
        );
    ...
    return factory.updateSourceFile(transformed, [
      factory.createImportDeclaration(
        ...
        factory.createStringLiteral("@windijs/helpers")
      ),
