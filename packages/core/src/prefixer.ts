...
export function prefixer(
  filter: ((k: string, v: string) => boolean) | string | Array<string>,
  updater: (k: string, v: string) => object
): CSSPrefixer {
  let updated;
...
export const prefixKeyframes = prefixer(["transform", "animationTimingFunction", "transformOrigin"], (k, v) => ({
  ["-webkit-" + camelToDash(k)]: v,
}));

export const prefixAnimation = prefixer("animation", (_, v) => ({ "-webkit-animation": v }));

export const prefixImageRendering = prefixer("imageRendering", (_, v) =>
  v === "pixelated"
    ? {
...
export const prefixNotHidden: CSSPrefixer = <T extends object>(t: T) => ({
  "& > :not([hidden]) ~ :not([hidden])": t,
});

export const prefixPlaceholder: CSSPrefixer = <T extends object>(t: T) => ({
...
export const prefixWritingMode = prefixer("writingMode", (_, v) => ({
