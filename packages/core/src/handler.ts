import { CSSMap, CSSObject, prop, StyleObject } from "@windijs/helpers";
...
export function fontFamilyHandler<T extends object>(fonts: T) {
  ...
  for (const [key, value] of Object.entries(fonts)) cssFonts[key as keyof typeof fonts] = Array.isArray(value) ? value.join(",") : (value as string);
