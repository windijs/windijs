...
export function hslToRGB(hsla: HSLA): RGBA {
  ...
  if (s === 0) r = g = b = l; // grayscaled color
  else {
  ...
  return [r, g, b]
    ...
    .concat([hsla[3]]) as RGBA;
}

export function hwbToRGB(hue: number, whiteness: number, blackness: number, alpha = 1): RGBA {
...
function adjustWithScale(rgba: RGBA | HSLA, deg?: number, scale = 0, index: 0 | 1 | 2 | 3 = 0, len = 100): RGBA | HSLA {
  rgba = [...rgba];
  rgba[index] = deg
...
export const adjustSaturation = (hsla: HSLA, deg?: number, scale = 0): HSLA => adjustWithScale(hsla, deg, scale, 1);

export const adjustLightness = (hsla: HSLA, deg?: number, scale = 0): HSLA => adjustWithScale(hsla, deg, scale, 2);

export const adjustRed = (rgba: RGBA, deg?: number, scale = 0): RGBA => adjustWithScale(rgba, deg, scale, 0, 255);

export const adjustGreen = (rgba: RGBA, deg?: number, scale = 0): RGBA => adjustWithScale(rgba, deg, scale, 1, 255);

export const adjustBlue = (rgba: RGBA, deg?: number, scale = 0): RGBA => adjustWithScale(rgba, deg, scale, 2, 255);

export const adjustAlpha: ((hsla: HSLA, deg?: number, scale?: number) => HSLA) & ((hsla: RGBA, deg?: number, scale?: number) => RGBA) = (
  hsla,
  deg,
  scale = 0
) => {
...
export function mixColor(color1: RGBA, color2: RGBA, w = 50): RGBA {
  const weightScale = w / 100;
  const normalizedWeight = weightScale * 2 - 1;
  const alphaDistance = color1[3] - color2[3];

  const combinedWeight1 =
    normalizedWeight * alphaDistance === -1 ? normalizedWeight : (normalizedWeight + alphaDistance) / (1 + normalizedWeight * alphaDistance);
...
export function subMixColor(color1: RGBA, color2: RGBA, w = 50): RGBA {
...
export class Color {
  ...
  static hwb(hue: number, whiteness: number, blackness: number, alpha = 1) {
  ...
  static mix(c1: Color, c2: Color, w = 50): Color {
    return Color.rgba(...mixColor(c1.rgbaval, c2.rgbaval, w));
  }

  static subcolormix(c1: Color, c2: Color, w = 50): Color {
  ...
  adjustRed(deg?: number, scale = 0): Color {
    return Color.rgba(...adjustRed(this.rgbaval, deg, scale));
  }

  adjustGreen(deg?: number, scale = 0): Color {
    return Color.rgba(...adjustGreen(this.rgbaval, deg, scale));
  }

  adjustBlue(deg?: number, scale = 0): Color {
  ...
  adjustSaturation(deg?: number, scale = 0): Color {
    return Color.hsla(...adjustSaturation(this.hslaval, deg, scale));
  }

  adjustLightness(deg?: number, scale = 0): Color {
    return Color.hsla(...adjustLightness(this.hslaval, deg, scale));
  }

  adjustAlpha(deg?: number, scale = 0): Color {
  ...
  adjust(opt: AdjustColorOptions): Color {
    ...
    for (const [k, v] of Object.entries(opt)) color = color[("adjust" + k[0].toUpperCase() + k.slice(1)) as AdjustColorFuncs](v);
  ...
  scale(opt: ScaleColorOptions): Color {
    ...
    for (const [k, v] of Object.entries(opt)) color = color[("adjust" + k[0].toUpperCase() + k.slice(1)) as ScaleColorFuncs](undefined, v);
