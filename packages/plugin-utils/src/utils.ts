...
export const filterConflict = (src: string, entries: (string | [string, string])[]) =>
  entries.filter(i => !src.match(new RegExp(`(const|let|var)\\s+${Array.isArray(i) ? i[1] : i}\\s*=`)));

export const injectImports = (code: string, imports: Record<string, (string | [string, string])[]>) =>
  Object.entries(imports)
    .map(([k, v]) => `import { ${v.map(i => (Array.isArray(i) ? i.join(" as ") : i)).join(", ")} } from '${k}';\n`)
    .join("") + code;
...
export const requireImports = (code: string, imports: Record<string, string[]>) =>
  code.replace(
    "var ",
    Object.entries(imports)
      .map(([k, v]) => `var { ${v.join(", ")} } = require('${k}');\n`)
      .join("") + "var "
  );

export const requireConfig = (code: string, path: string | false) =>
  path ? code.replace("var ", `var windiUserConfig = require('${path}');\nvar `) : code;

export const requireHelper = (code: string, helper: string, pkg: string) =>
  code.includes(helper) ? code : requireImports(code, { [isProduction ? pkg : "windijs"]: [helper] });
...
export const declModule = (name: string, contents: string | string[]) =>
...
export const updateEnv = (entry: string | false, content: string | undefined) => {
  ...
  if (content)
    writeFileSync(
      entry,
      "// This file was auto generated by windijs, you can put it into your `.gitignore`, please do not edit it directly.\n" + content
    );
  else if (existsSync(entry)) rmSync(entry);
};

export function readPackage(dir: string) {
  return JSON.parse(readFileSync(join(dir, "package.json")).toString()) as {
    types?: string;
    module?: string;
    main?: string;
...
export function resolveOptions(options: PluginOptions): ResolvedPluginOptions {
  ...
  if (options.config == null || !testConfigEntry(resolvedOptions.env.configEntry)) resolvedOptions.env.configEntry = false;
  // didn't set config, disable config entry.
