...
Variant allows you to specify under what circumstances your utilities will be activated.
You may use the screen size, system appearance, or any pseudo-selector, such as `:hover`. You can use variant just like ordinary functions in Windi JS.
...
All built-in variants are defined in [@windijs/variants](https://github.com/windijs/windijs/tree/main/packages/variants). 

**Note**: Variants should follow **camelCase** naming convention, because dashCase cannot be used to name variables in JavaScript, so `not-checked -> notChecked`, also they shouldn't start with numbers, for example `2xl -> xxl`, which is different from Windi CSS.
...
Create variants by configuration object.

:::warning
This api may be renamed to `setupVariants` from next version.
:::
...
Usually, you use variant like a function. However, in some special cases, you may want to use custom variant quickly without specifying them as variables or putting them in config file. The following Apis can help you complete this work.
