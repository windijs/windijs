...
import { useMonaco, htmlModel, styleModel, scriptModel, globalModel, TypeScriptWorker } from "$/monaco";
...
let tsProxy: TypeScriptWorker;
let mainEditor: editor.IStandaloneCodeEditor;
let renderEditor: editor.IStandaloneCodeEditor;
let configEditor: editor.IStandaloneCodeEditor;
...
const btnStyle = [
  ...
  text.gray[400],
  hover(text.gray[500]),
  dark(bg.dark[400].opacity(30), text.stone[600], hover(text.stone[500])),
];

const select: Ref<HTMLSelectElement | null> = ref(null);

function apply(selector: string, key: string, value: string | null) {
...
onMounted(() => {
  useMonaco().then(({ monaco, editor, renderEditor: _renderEditor, configEditor: _configEditor }) => {
    ...
    const updateConfig = (proxy: TypeScriptWorker) => {
      const model = configEditor.getModel();
      if (model)
        proxy.getEmitOutput(model.uri.toString()).then(r => {
    ...
    monaco.languages.typescript.getTypeScriptWorker().then(worker => {
      ...
      const configModel = configEditor.getModel();

      editorModel && worker(editorModel.uri).then(updateScript);
      configModel && worker(configModel.uri).then(updateConfig);
    });
  ...
  split = Split(["#editor", "#render"], {
    ...
    gutterAlign: "center",
    onDrag: sizes => {
      if (window.outerWidth < 640) apply("#render", "display", null);
      apply(".monaco-editor", "display", sizes[0] < 1 ? "none" : null);
    },
  });
});
...
function decorate(src: string, desc: string) {
...
function updateRender(tab: TabIndex = -1) {
  currentTab = tab;
  if (tab === 0) {
    const iframe = window.frames[0].document.getElementById("app");
    if (iframe) htmlModel.setValue(iframe.innerHTML.replace(/></g, ">\n<").replace(/></g, ">\n<"));
    if (renderEditor) {
      ...
      renderEditor
        ...
        .run()
        .then(() => renderEditor.updateOptions({ readOnly: true }));
    }
    showRenderEditor.value = true;
  } else if (tab === 1) {
    const windijs = window.frames[0].document.getElementById("windijs")?.innerHTML.replace(/^\s*/, "");
    const windicss = window.frames[0].document.getElementById("windicss")?.innerHTML;

    const allCSS =
      (windicss ? decorate(windicss, "Generate by Windi CSS v4.0") : "") +
      (windijs ? (windicss ? "\n\n" : "") + decorate(windijs, "Generated by Windi JS v1.2") : "");
    styleModel.setValue(allCSS);
...
function hideRenderEditor(tab: TabIndex) {
  ...
  if (json !== prevConfig) {
    ...
    if (added.length > 0) {
      ...
      globalModel.setValue(
        value
          .replace(/(import\s+)/, 'import { StyleObject, VariantBuilder } from "@windijs/helpers";\n$1')
          .replace(/(declare\s+global\s*{)/, "$1\n" + added.map(i => "  " + i + ";\n").join(""))
      );
    }
