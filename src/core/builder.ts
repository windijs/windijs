...
export function buildColor (colorProperty: StyleProperties | StyleProperties[], colorOpacityProperty: undefined, value: unknown, prefixer?: (css: CSSObject) => CSSObject): StyleObject;
export function buildColor (colorProperty: StyleProperties | StyleProperties[], colorOpacityProperty: string, value: unknown, prefixer?: (css: CSSObject) => CSSObject): ColorStyleObject;
export function buildColor (colorProperty: StyleProperties | StyleProperties[], colorOpacityProperty: string | undefined, value: unknown, prefixer: CSSPrefixer = i => i): StyleObject | ColorStyleObject | undefined {
  ...
  if (colorOpacityProperty != null) {
    if (value.startsWith("#")) {
      ...
      decl = {
        ...buildProperty(colorProperty, value.startsWith("hwb") ? parenWrap("hwb", values.slice(0, 3).join(" ") + " / " + parenWrap("var", colorOpacityProperty)) : parenWrap(value.startsWith("hsl") ? "hsla" : "rgba", [...values.slice(0, 3), parenWrap("var", colorOpacityProperty)].join(", "))),
    ...
    const gradient = (obj: CSSObject) => ({
      get gradient () {
        return css(prefixer({ ...obj, backgroundImage: "linear-gradient(180deg, rgba(255, 255, 255, 0.15), rgba(255, 255, 255, 0))" }));
      },
    });
...
function buildODir (v: string): string {
  return [v.includes("top") ? "bottom" : v.includes("bottom") ? "top" : null, v.includes("left") ? "right" : v.includes("right") ? "left" : null, v.endsWith("deg") ? Math.abs(450 - (+v.slice(0, -3))) % 360 + "deg" : null].filter(i => i != null).join(" ");
}

function buildWebkitGradient (v: string, colorStops: CSSLinearColorStopOrHint[]): string {
  const len = colorStops.length;
  const dirs = v.split(" ");
  const stops = colorStops.map((stop, i) => (i === 0 && typeof stop === "string") ? parenWrap("from", stop) : (i === len - 1 && len > 1 && typeof stop === "string") ? parenWrap("to", stop) : parenWrap("color-stop", Array.isArray(stop) ? stop.reverse().join(", ") : stop.toString()));
...
export function buildGradientDirection (v: unknown, colorStops: CSSLinearColorStopOrHint[] = ["var(--w-gradient-stops)"]) {
  ...
  if (isTo()) d = "to " + v;
    ...
    d = v;
    switch (v) {
    case "0deg": v = "top"; break;
    case "90deg": v = "right"; break;
    case "180deg": v = "bottom"; break;
    case "270deg": v = "left"; break;
    }
  }
  ...
  return css({ backgroundImage: isTo() ? [buildWebkitGradient(v as string, colorStops), o, g] : [o, g] });
}
...
export function buildGradientFrom (v: unknown) {
  return buildColor(prop`--w-gradient-from`, "--w-from-opacity", v,
    (c) => ({ ...c, "--w-gradient-stops": "var(--w-gradient-from), var(--w-gradient-to, rgba(255, 255, 255, 0))" }),
  );
}

export function buildGradientVia (v: unknown) {
  return buildColor(prop`--w-gradient-via`, "--w-via-opacity", v,
    (c) => ({ "--w-gradient-stops": `var(--w-gradient-from), ${c["--w-gradient-via"]}, var(--w-gradient-to, rgba(255, 255, 255, 0))` }),
  );
}
...
export function buildBoxShadowColor (v: unknown) {
  return buildColor(prop`--w-shadow-color`, "--w-shadow-color-opacity", v, (o) => ({ ...o, "--w-shadow": "var(--w-shadow-colored)" }));
}
