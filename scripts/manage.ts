...
function relatedPackages (target: string) {
  ...
  for (const [target, pkg] of pkgs) {
    ...
    if (pkg.devDependencies && name in pkg.devDependencies) {
      devRelated.push(target);
    }

    if (pkg.peerDependencies && name in pkg.peerDependencies) {
      peerRelated.push(target);
    }
  }

  if (related.length === 0 && devRelated.length === 0 && peerRelated.length === 0) {
    console.log(target + " has no related pacakges");
  } else {
    console.log(target + " related packaged:");
  }

  if (related.length > 0) {
    console.log("- dependencies related " + "[" + related.join(", ") + "]");
  }

  if (devRelated.length > 0) {
    console.log("- devDependencies related " + "[" + devRelated.join(", ") + "]");
  }

  if (peerRelated.length > 0) {
    console.log("- peerDependencies related " + "[" + peerRelated.join(", ") + "]");
  }
}
...
function getFiles (dir: string) {
  return fs.readdirSync(dir, { encoding: "utf-8" }).filter(i => !i.startsWith(".") && fs.statSync(path.join(dir, i)).isFile()).map(i => path.join(dir, i));
}
...
function updateHashes (published: string[] | undefined = undefined) {
  ...
  if (published == null) {
    hashes = generateHashes(targets);
  } else {
    for (const target of published) {
      hashes[target] = generateHash(target);
    }
  }
...
function trackChanges (packages: string[]) {
  ...
  const changed = Object.entries(newHashes).filter(([k, v]) => (k in hashes) && hashes[k] !== v).map(([k]) => k);
...
function logChanges (added: string[], changed: string[], removed: string[]) {
  if (added.length > 0) {
    console.log(chalk.green(`- ${added.length} package${added.length === 1 ? "" : "s"} added [${added.join(", ")}]`));
  }

  if (changed.length > 0) {
    console.log(chalk.blue(`- ${changed.length} package${changed.length === 1 ? "" : "s"} changed [${changed.join(", ")}]`));
  }

  if (removed.length > 0) {
    console.log(chalk.red(`- ${removed.length} package${removed.length === 1 ? "" : "s"} removed [${removed.join(", ")}]`));
  }
